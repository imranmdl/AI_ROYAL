...
app.post('/api/sync', async (req: Request, res: Response) => {
  ...
  app.post('/api/admin/migrate-default-tenant', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const tables = ['products','sales','purchases','vendor_orders','users','loading_charges','gallery_leads'];
      const results: any = {};
      for (const table of tables) {
        try {
          const [r]: any = await pool.query(
            `UPDATE ${table} SET tenant_id=? WHERE tenant_id='default' OR tenant_id IS NULL OR tenant_id=''`,
            [tenantId]);
      ...
      inMemoryDb = null as any;
      res.json({ success:true, tenantId, slug, loginUrl:`/?tenant=${slug}`, migrated:results,
        message:`Default tenant migrated to "${shopName}" (slug: ${slug}).` });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
    ...
    try {
      ...
      res.json({
        ...
        user: {
          ...
          password:user.password, twoFactorEnabled:user.twoFactorEnabled||false,
        },
