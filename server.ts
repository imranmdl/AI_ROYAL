...
async function initDatabase(config: DbConfig = activeDbConfig) {
  try {
    ...
    for (const table of tables) {
      try {
        await connection.query(`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS updated_at BIGINT DEFAULT 0`);
      } catch (err) {
        // Some MySQL versions don't support ADD COLUMN IF NOT EXISTS
        // We can ignore errors here as the column might already exist
      }
    }
